...
앞 설명 없이 코드 블록이 이어지면 그 블록 **바로 앞**에 무엇을 하는 코드인지 **1문장**으로 안내한다.

- 형식: 코드 펜스(```` ```python ````) **바로 앞**에 평범한 한 문장(머릿말·이모지 없이), 위에 빈 줄. 같은 줄에 코드 시작 금지.
- 근거: 코드 자체 + 노트북 마크다운. 이미 앞에 충분한 산문이 있으면 **중복 추가 금지**.
- 건너뜀: `import`·`!pip install`·`!nvidia-smi` 같은 사소한 셀은 설명하지 않는다.
- 톤: 존댓말·간결, 동작 중심(예: "별점을 0-1 float 로 바꿔 회귀 라벨을 만듭니다").
