...
C5 는 *후보* 만 뽑는다. 실행마다 값이 달라지는 metric 은 결함이 아니므로
(가이드의 "실측과 다른 수치" 는 재현 가능한 값이 어긋난 경우를 말한다) 사람이
판단해야 한다. 자동으로 결함 처리하지 말 것.

다만 **소요 시간은 결함 판정 대상이다** (2026-08-21 확정). 도입부의 "예상 소요
시간"이 실측(Trainer 표의 `[626/626 00:37]`)과 배수 단위로 어긋나면 결함으로
올린다. 이 스크립트는 시간 문구를 잡지 않으므로 사람이 직접 대조해야 한다.
