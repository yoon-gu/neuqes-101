...
BERT가 macro F1에서 +0.19(0.61 → 0.80)로 가장 크게 앞서고, 이득의 대부분은 macro recall(+0.19)에서 옵니다 — 드문 라벨을 BERT가 훨씬 잘 잡습니다. 다만 macro AUC는 sklearn이 0.02 높아, 키워드가 본질 신호인 합성 라벨에서는 TF-IDF의 확률 정렬력도 만만치 않음을 보여줍니다.
...
BERT는 드문 라벨에서 압도적입니다 — location +0.40, ambiance +0.35, price +0.21로, 키워드만으로는 안 잡히는 신호까지 학습한 결과입니다. 반대로 service는 sklearn이 0.04 앞서는데, 키워드가 *결정적* 인 라벨에서는 TF-IDF의 정확한 매칭이 BERT의 근사보다 유리할 수 있음을 보여줍니다.
