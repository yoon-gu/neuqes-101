...
균등분포 baseline ln(V)=7.62, 그리고 유니그램만 외운 collapse 모델의 정체값(약 6.0)보다 한참 아래인 **3.59**까지 내려갔습니다. 모델이 단순 빈도가 아니라 문맥 의존 구조를 학습했다는 신호이고, 30000 step이 T4 30분 예산 안에 들어왔습니다. (모델·학습은 Ch 32와 동일 — 이 장이 바꾸는 건 아래의 샘플러입니다.)
