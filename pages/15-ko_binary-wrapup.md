...
힌트: *코드는 에러 없이 돌아갑니다.* 다만 정확도가 한국어 모델에 크게 못 미칩니다 — 영어 vocab 에 한국어 글자가 없어 문장이 *자모 단위로* 쪼개지고 `[UNK]` 도 많이 섞입니다. 그래도 같은 글자는 늘 같은 자모 열이 되므로 모델이 *문자열 패턴* 으로 약한 신호는 잡아냅니다 — 의미를 이해한 게 아니라는 점이 핵심입니다. *Phase 2 의 핵심 교훈* — 한국어엔 *반드시* 한국어 토크나이저 + 한국어 사전학습 모델.
