...
|---|---|---|---|
| `N_TRAIN_TEXT` | 5,000 | 30,000 | loss 하락폭 큼. 학습 자체는 1 epoch 약 1분 수준 (실측 5K 2 epoch 0.3분 × 약 6배) — 30분 룰의 상한은 학습이 아니라 다운로드·토큰화 |
| `num_train_epochs` | 2 | 3 | eval loss 소폭 하락 — 작은 코퍼스라 과적합 주의, 효과는 셀 출력으로 확인 |
...
> **T4 30분 룰 안에서 가능한 가장 큰 개선** — 학습 시간만 보면 `N_TRAIN_TEXT` 를 수만 단위로 늘려도 여유가 큽니다 (상한은 다운로드·토큰화와 메모리). Ch 23 에서 *얼마나 도움 됐는지* 가 진짜 검증.
