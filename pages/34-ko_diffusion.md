직전 장에서 작은 mask-diffusion 언어 모델을 영어 TinyStories로 학습해, 인물과 대화와 배경을 갖춘 이야기를 끝까지 이어가게 만들었습니다. 그 레시피를 그대로 한국어 데이터로 옮기면 같은 결과가 나올 것 같지만, 실제로는 처음 몇백 step 만에 손실이 한국어 유니그램 엔트로피 근처에서 멈춰 버리고, 생성은 ",. 함께 하루, 큰.!." 같은 구두점과 고빈도 파편만 토해 냅니다. 모델이 문맥을 보고 빈칸을 맞히는 대신, "[MASK]를 보면 그냥 흔한 토큰을 찍는" 지름길에 빠진 것입니다. 이 장에서는 그 붕괴를 진단하고, 데이터·용량·학습량이 아니라 **마스킹 방식**이 범인임을 대조 실험으로 좁힌 뒤, BERT가 원래 쓰던 80/10/10 마스킹 트릭으로 살려 냅니다.
...
| **Ch 34 (해결)** | **한국어 (TinyStories-Korean)** | **ByteLevel BPE, vocab 4000** | **80/10/10 (`[MASK]` / 랜덤 / 원본)** | **순진한 이식은 train_loss 7.06 · acc 0.081로 붕괴 → 80/10/10으로 train_loss 4.11 · acc 0.651 회복, 생성 coherent** |
...
## 마스킹 노트 — 80/10/10이 한국어 diffusion을 살린다

직전 챕터의 영어 diffusion은 마스킹 대상으로 뽑은 자리를 **100% `[MASK]`** 로 바꿔도 잘 동작했습니다. 그런데 같은 레시피를 한국어로 옮기자 학습이 처음 250 step만에 멈춰버렸습니다. `train_loss`가 7.06에서 더 내려가지 않았는데, 이 값은 한국어 유니그램 엔트로피(토큰 빈도만 보고 찍을 때의 손실)와 거의 같습니다. 아무 문맥 없이 균등하게 찍으면 손실이 $\ln 4000 \approx 8.29$이니, 7.06은 거기서 빈도 정보만큼만 내려온 값입니다. 모델이 "`[MASK]`가 보이면 그냥 제일 흔한 토큰을 찍는다"는 지름길만 배우고 멈춘 셈입니다.

### 왜 100% `[MASK]`가 지름길을 만드는가

입력에 `[MASK]`가 들어오는 자리에서만 손실이 걸린다면, 모델은 두 가지 사실을 알아챌 수 있습니다. 첫째, 정답을 맞혀야 하는 자리는 **항상** `[MASK]` 토큰이 놓인 자리입니다. 둘째, `[MASK]` 자체는 아무 의미가 없으니 주변 문맥만 보면 됩니다. 한국어는 조사("-가", "-를", "-에")와 어미("-요", "-어요", "-습니다")의 빈도가 압도적으로 높아, 문맥을 거의 보지 않고 고빈도 토큰만 찍어도 손실이 빠르게 떨어집니다. 그러니 모델은 굳이 문맥을 더 파고들 이유를 못 느끼고 유니그램 수준에서 정체합니다.

### 80/10/10이 지름길을 막는 방식

BERT가 쓰는 트릭은 마스킹 대상으로 뽑힌 자리를 셋으로 나눕니다.

- **80% → `[MASK]`** — 일반적인 빈칸 복원
- **10% → 랜덤 토큰** — 엉뚱한 토큰으로 바꿔놓고 정답을 맞히게 함
- **10% → 원본 그대로 유지** — 입력은 멀쩡한데 그 자리에도 손실이 걸림

핵심은 두 소수파입니다. **10% 원본 유지**는 "정답을 맞혀야 하는 자리가 꼭 `[MASK]`인 것은 아니다"라고 알려줍니다. 모델은 `[MASK]` 표식에만 의존할 수 없게 되어, 멀쩡해 보이는 토큰도 문맥과 맞는지 매번 검증해야 합니다. **10% 랜덤 토큰**은 "눈에 보이는 입력이 틀렸을 수 있으니 교정하라"고 강제합니다. 입력 토큰을 그대로 믿는 대신 문맥과 대조하는 습관이 생깁니다. 두 장치가 합쳐지면 "`[MASK]` 보면 고빈도 토큰 찍기"라는 지름길이 막히고, 모델은 어쩔 수 없이 문맥을 읽기 시작합니다.

영어 직전 챕터가 100% `[MASK]`로도 버틴 이유는 데이터가 단순하고 반복적이라 유니그램 지름길의 유혹이 약했기 때문입니다. 한국어는 조사와 어미의 유니그램 예측성이 높아 이 트릭이 선택이 아니라 필수가 됩니다.
...
표를 보면 $t=0.15$에서 정답을 맞혀야 하는 20개 자리 중 4개(랜덤 2 + 원본 2)는 `[MASK]` 표식이 아예 없습니다. 이 4개가 모델에게 "표식이 아니라 문맥을 보라"고 끊임없이 압박하는 장치입니다. 손실은 이 마스킹 대상 자리들의 평균 교차 엔트로피로 계산합니다. 직전 챕터에서 쓰던 시간 가중 $1/t$는 기댓값 상 마스크 토큰당 평균 CE와 상수배로 같으므로, 여기서는 표준 MLM과 눈높이를 맞추기 위해 가중 없는 평균 CE를 그대로 씁니다.

이 수정만으로 7.0의 벽이 무너집니다. `train_loss`는 4.11로 내려가고, 고정 $t=0.15$에서 top-1 정확도는 0.081에서 0.651로 뛰며, 생성도 인물과 서사를 갖춘 문장으로 살아납니다.
...
순진한 이식에서는 `train_loss`가 처음 250 step 만에 7.0 근처로 주저앉은 뒤 끝까지 움직이지 않았습니다. 유니그램 분포만 외우고 학습이 멈춰버린 자리입니다. 80/10/10과 plain CE로 콜레이터만 바꿨더니 같은 모델, 같은 데이터, 같은 step 예산에서 이 벽을 그대로 통과합니다.

| 구성 | 마스킹 | train_loss | 고정-t(0.15) top-1 acc |
|---|---|---|---|
| 순진한 diffusion (Ch 33 레시피) | 100% [MASK] | 7.06 | 0.081 |
| 수정판 (로컬 8000 step) | 80/10/10 | - | 0.467 |
| 수정판 (Colab T4 30000 step) | 80/10/10 | 4.11 | 0.651 |
| (대조) 표준 MLM 15% | 80/10/10 | - | 0.469 |
| (참고) 영어 Ch 33 | 100% [MASK] | - | 0.717 |

`train_loss`는 baseline $\ln(4000) = 8.29$에서 시작합니다. 순진한 버전의 7.06은 거기서 1.2밖에 못 내려간, 사실상 한국어 유니그램 엔트로피 근처입니다. 수정판의 4.11은 모델이 비로소 문맥을 읽기 시작했다는 신호입니다. 로컬 8000 step만으로도 acc가 0.467까지 올라 표준 MLM(0.469)과 사실상 같은 자리에 도달했고, Colab에서 30000 step을 채우면 0.651까지 갑니다. 영어 Ch 33의 0.717과 비교하면, 교착어인 한국어가 조금 더 어렵지만 같은 레시피가 같은 방식으로 작동한다는 점이 핵심입니다.
...
- [34-1. 실습 — 80/10/10 수정판 직접 학습하기](34-ko_diffusion-practice.md)
- [34-2. 해부 — 붕괴를 진단하고, 범인을 좁혀가기](34-ko_diffusion-anatomy.md)
