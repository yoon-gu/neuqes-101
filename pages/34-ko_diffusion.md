...
인물과 대화, 배경과 서사가 모두 들어 있습니다. 고정 마스킹 비율 $t=0.15$에서 빈칸 복원 top-1 정확도는 0.652까지 올라가고(영어 Ch 33은 약 0.71), `train_loss`는 4.13까지 내려갑니다.
...
|---|---|---|---|---|
| Ch 33 (성공) | 영어 (TinyStories) | ByteLevel BPE, vocab 2048 | 100% `[MASK]` | train_loss 낮고 생성 coherent, top-1 acc 약 0.71 |
| **Ch 34 (해결)** | **한국어 (TinyStories-Korean)** | **ByteLevel BPE, vocab 4000** | **80/10/10 (`[MASK]` / 랜덤 / 원본)** | **순진한 이식은 train_loss 7.06 · acc 약 0.08로 collapse → 80/10/10으로 train_loss 4.13 · acc 0.652 회복, 생성 coherent** |
...
이 한 가지 수정으로 `train_loss`가 4.13까지 내려가고 고정-$t$(0.15) top-1 정확도가 약 0.08에서 0.652로 뜁니다. 곧 실습에서 직접 확인합니다.
