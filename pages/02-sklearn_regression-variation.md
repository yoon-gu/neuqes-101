...
**위 코드 읽기** 예측을 `* 4 + 1` 로 별점 공간에 되돌려 정규화 없이 학습한 모델과 같은 척도로 비교합니다. 두 값이 같다면 라벨 정규화가 최소제곱법 해를 바꾸지 않았다는 뜻입니다.
...
별점 공간으로 되돌린 MSE가 정규화 없이 학습한 1.5565와 정확히 같습니다. `LinearRegression` 은 최소제곱법으로 풀기에 라벨 스케일링이 최종 예측에 영향을 주지 않음을 보여줍니다.
