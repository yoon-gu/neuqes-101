...
4. 같은 한국어 데이터로 AR(Ch 26)은 coherent 생성에 성공했고 표준 MLM도 고정-$t$ 정확도 0.469를 냈는데, 순진한 diffusion만 0.081로 무너졌습니다. 이 대조군 세 개를 나란히 놓는 것이 "데이터·토크나이저가 아니라 마스킹이 범인"이라는 진단에 왜 결정적인지 설명해 보세요.
...
이로써 diffusion 라인의 핵심 골격은 갖춰졌습니다. 다음으로는 지금까지 손으로 쌓아 온 자기회귀(AR) 계열과 diffusion 계열을 한 호흡으로 되돌아보며 두 패러다임을 같은 잣대로 정리합니다. 학습 신호의 밀도(AR은 매 step 모든 위치를 감독, diffusion은 마스킹된 일부 자리만), 생성 방식(AR은 좌→우 순차, diffusion은 양방향 병렬 채우기), 그리고 같은 규모·데이터에서 드러난 학습 난이도 차이를 나란히 놓고, 어느 쪽이 어떤 상황에 맞는지를 회고합니다.
