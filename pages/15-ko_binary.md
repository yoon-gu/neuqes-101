...
영어 토크나이저는 한국어를 *낯선 문자열* 로 보고 자모(초·중·종성) 단위까지 쪼갭니다 (위 예시는 14 토큰). 한국어 토크나이저는 *재미있·었·어요* 를 어휘적 의미 단위로 분할 — 모델이 임베딩을 통해 *의미* 를 잡을 수 있는 형태.
...
- 영어 토크나이저는 한국어를 *자모(초·중·종성) 단위* 로 쪼개거나 (`ᄋ`, `##ᅵ`, `##ᅧ`) `[UNK]` 로 처리 — 의미를 못 잡음
