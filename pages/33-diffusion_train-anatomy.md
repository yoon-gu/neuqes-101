...
### 세 숫자를 한 장에 — Ch 32 붕괴 대비

직전 장의 붕괴와 이번 장의 교정을 같은 잣대로 나란히 놓으면, 무엇이 어떻게 달라졌는지가 한눈에 들어옵니다.
...
세 숫자는 같은 이야기를 세 각도에서 합니다. top-1 accuracy 0.717는 모델이 가려진 자리를 양방향 문맥으로 복원한다는 것을, KL 0.78은 그 결과가 단순 빈도표가 아니라 문맥 의존 구조라는 것을, 4-gram 0.000은 그 구조가 디코딩 단계에서 반복 루프로 무너지지 않는다는 것을 보여줍니다. 직전 장이 "왜 망가졌나"를 눈대중으로 물었다면, 이 장은 "정말 고쳐졌나"를 숫자로 답합니다.
...
고정-t 정확도 0.717은 15%만 가린 자리 10개 중 7개를 양방향 문맥만으로 복원한다는 뜻으로, 모델이 조건부 구조를 실제로 익혔다는 직접 증거입니다. 반복억제 샘플러는 4-gram 반복을 0.177에서 0으로 떨어뜨립니다. 모델은 구조를 배웠고, 샘플러는 그 구조를 같은 조각으로 망치지 않습니다.
