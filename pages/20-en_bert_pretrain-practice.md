...
한 epoch 이 170 step 남짓, 2 epoch 을 다 돌아도 336 step 뿐입니다 — 사전학습이라기엔 대단히 짧은 분량이고, 뒤에서 loss 가 unigram 기준선 근처에 머무는 이유도 여기서 이미 예고됩니다. 출력의 `167` 은 `//`(몫) 계산이라 한 칸 적게 나온 값으로, `Trainer` 는 남는 마지막 batch 도 버리지 않고 돌려 실제로는 epoch 당 168 step 입니다.
