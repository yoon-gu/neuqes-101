...
두 동기는 *적절한 λ 가 전제* 일 때 작동합니다. 이번 본문은 λ=1 한 점만 보여 주는데, λ=1 이 과하면 보조가 메인을 잠식해 오히려 F1 이 떨어질 수 있습니다(실제로 이번 실행이 그렇게 나옵니다). 정규화·데이터 효율 효과는 λ 를 0.1·0.3 등으로 낮춰 grid search 했을 때 드러납니다.
...
이번 λ=1 실행에서는 *모든 metric delta 가 음수* 입니다 — micro-F1 −0.17, macro-F1 −0.42 로 메인이 크게 하락. λ=1 이 과해 보조 회귀가 공유 본체를 잠식한 것이며, λ 를 줄여(0.1, 0.3) grid search 하면 양상이 달라집니다.

### 8-2. 라벨별 F1 비교 — 어느 항목이 보조 loss로 가장 도움받았나

**해석**

- **이번 λ=1 실행에서는 모든 항목 delta가 음수** — food·service 도 약하게 떨어지고(약 −0.03, −0.08), 활성률이 낮은 price·ambiance·location 은 F1 이 거의 0 으로 무너집니다(약 −0.46, −0.69, −0.82). λ=1 이 과해 보조 회귀가 공유 본체를 별점 쪽으로 강하게 끌어당긴 결과입니다.
- **활성률이 낮은 항목일수록 타격이 큼** — 학습 신호가 약한 항목부터 보조 항에 잠식돼 분류를 사실상 포기합니다.
...
이 챕터의 §8 비교는 *드라마틱한 양의 delta* 가 항상 나오는 셋업이 아닙니다. 실제로 이번 λ=1 풀 모드 실행(5,000 train / 1,000 eval / 2 epoch)에서는 *모든 항목 delta 가 음수* 로 나왔습니다 — λ=1 이 과해 보조가 메인을 강하게 잠식한 결과입니다. 아래 시나리오로 *어떤 패턴이 왜 나오는지* 를 먼저 정리합니다.
...
| **일부 항목만 향상, 나머지 0 또는 음수** | 별점과 *상관 강한* 항목 (food/service) 만 보조의 도움을 받음. 활성률이 낮은 항목 (price/ambiance/location) 은 *학습 데이터 자체가 부족* 해 둘 다 학습 안 됨 | 데이터·에폭 늘리기가 우선, λ 보다 더 큰 lever |
...
| **모든 항목 음수** ← *이번 λ=1 실행의 결과* | 보조가 메인 학습을 *방해* — λ=1 이 과해 보조 회귀가 공유 본체를 잠식 (FAQ Q4) | λ 줄이거나(0.1, 0.3) grid search. 이게 RoBERTa 가 NSP 를 뺀 이유 |

### 우리 결과를 *솔직하게* 읽기

§8-2 의 라벨별 F1 표 가 *어떤* 패턴인지 확인:

- **모든 항목 delta 가 음수** → 마지막 시나리오. food −0.029·service −0.084 처럼 상관 강한 항목도 약하게 떨어지고, 활성률이 낮은 price −0.461·ambiance −0.688·location −0.820 은 F1 이 거의 0 으로 무너집니다. λ=1 이 과해 보조 회귀가 공유 본체를 별점 쪽으로 끌어당겨 학습 신호가 약한 항목의 분류를 사실상 포기시킨 결과입니다.
- **메인 전체도 강하게 하락** → micro-F1 0.8163 → 0.6438, macro-F1 0.7577 → 0.3414. macro AUC 만 0.9081 → 0.8073 으로 비교적 덜 떨어집니다 — 임계값 기반 F1 이 AUC 보다 보조 잠식에 더 민감하기 때문.
- **보조 task 자체** (별점 회귀 Pearson 0.81) 는 *오히려 잘 학습* 됨. 보조가 *작동하지 않아서* 메인이 망가진 게 아니라, λ=1 에서 보조가 너무 세게 학습되며 공유 본체를 자기 쪽으로 끌어가 메인을 밀어낸 것입니다.

### 그럼 auxiliary loss 의 가치는 어디에서?

이번 λ=1 결과만 보면 "오히려 해롭잖아?" 싶을 수 있습니다. λ=1 이 과해 보조가 메인을 잠식한 것이고, λ 를 줄이거나(0.1, 0.3) grid search 하면 양상이 달라집니다 — 다음과 같은 *다른 측면* 도 같이 봐 주세요:
...
3. **데이터 효율 누적 효과** — 별점 같은 *공짜 메타데이터* 신호를 추가 라벨링 비용 없이 활용. 데이터셋이 *크면 클수록* 효과 누적. 단 적절한 λ 가 전제이며, λ 가 과하면 본문처럼 메인이 잠식됩니다.
...
> **원고 취지** — auxiliary loss 라는 *기법 자체* 와 *언제 도움이 되는지* 의 이해. λ=1 단일 실행으로 "보조 loss 는 좋다 / 나쁘다" 를 단정하기보다, *언제 효과가 나타나는 패턴인지* 의 감각을 잡는 게 이 챕터의 목적. 이번처럼 *λ 가 과해 메인이 망가진 결과* 도 진단할 수 있는 게 더 중요한 학습 결과입니다.
