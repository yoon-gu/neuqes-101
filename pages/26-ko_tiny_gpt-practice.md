...
prompt: 작은 소녀가
작은 소녀가 소녀가 �덩 싫어 쳐다 개미는 남하게 친한 자기 꾸 꾸 불� 도와주n��우인지머 자동차님은 정원을 노인 맜�니 수 잃 낮잠을니� 매우 놀았어요 놀았어요 너희 너희 대해혜에는 빨리 친한 몰랐습니다 몰랐습니다 나뭇가지를게요 하자� 가는장에서건 …(뒤 28자 생략)
prompt: 큰 개가
큰 개가보�� 않아요 그려�브�날 우유 우유 하지만 아이들이 벽 곰을 사실에 침 물었습니다이에요 의� 옛날 돌려주 되어 다가��티라 작겁물론 엄 소녀가 그녀를 나�븐 말했습니다 방법�도와났어요�� 산책을팡 못 분홍 프 말씀 몸을 몸을�� 물고기를치 크 …(뒤 27자 생략)
prompt: 어느 날,
...
random init 상태라 네 prompt 모두 의미 없는 음절·byte 조각(`개구리가보�려주정말돌돌��`, `도와주n��우인지머` 등)이 뒤죽박죽 나열됩니다. `놀았어요 놀았어요`, `몰랐습니다 몰랐습니다`처럼 BBPE vocab의 어절 토큰이 통째로 튀어나오기도 하지만 앞뒤 문맥과는 무관합니다. logits가 무작위 초기값이라 next-token 분포가 형성되지 않은 상태로, 학습 후 출력과 비교할 baseline이 됩니다.
...
## `Trainer` 로 사전학습 (Ch 24 와 같은 셋업, 학습률만 5e-4)
...
1500 step 학습이 약 0.91분에 끝났고, 누적 평균 `train_loss`가 random baseline 8.29에서 4.46까지 내려갔습니다. 번역체 한국어라 영어 챕터(Ch 24, 약 3.7)보다 다소 높지만(학습률도 `3e-4 → 5e-4`로 달라 언어 차이만의 효과는 아님) baseline에서 절반 가까이 떨어진 정상 수렴입니다. 마지막 줄의 `final peak` 66 MiB는 학습 중 최대치가 아닙니다 — VRAM 콜백이 로그마다 peak 기록을 초기화하므로, 학습이 끝난 뒤 GPU에 남아 있는 모델 가중치·optimizer 상태 정도의 메모리입니다. 학습 중 peak는 다음 그래프의 VRAM trace에서 확인합니다.
...
![output](../assets/26-ko_tiny_gpt-out1-2.png)

**결과 해석**

왼쪽 그래프에서 train loss가 random baseline(점선 `ln(4000)≈8.29`) 바로 아래에서 시작해 300 step 무렵 약 5 아래로 급락한 뒤, 완만히 내려가 1500 step에서 train 약 3.8·eval 약 3.7에 닿습니다. eval loss가 모든 평가 지점에서 train loss보다 낮아 과적합 징후는 없습니다. 앞 셀 summary의 `train_loss` 4.46은 1500 step 전체의 누적 평균이라 그래프 끝 값보다 높게 나옵니다. 오른쪽 VRAM trace는 학습 구간마다 600 MiB 남짓, 평가 구간마다 약 230 MiB로 오르내리는 톱니 모양이고, 맨 끝의 약 66 MiB는 학습 종료 뒤 남은 메모리입니다. 학습 peak도 T4 VRAM(약 14.56 GiB)의 5% 미만이라 작은 GPT + T4 조합은 메모리 측면에서 매우 여유롭습니다.
...
======================================================================
prompt: 옛날 옛날에
옛날 옛날에 잭이라는 작은 소녀가 있었습니다. 그녀는 가장 좋아하는 꽃들로 있었어요. 어느 날, 루시는 그 집으로 갔고, 수라는 이름의 작은 소녀가 공원에 갔어요. 수라는 이름의 소녀가 친구인 소녀가 그 새를 봤어요. 수라는 그 새를 돕고 싶어졌습니다 …(뒤 37자 생략)
prompt: 작은 소녀가
