...
print(f"Best threshold (Youden's J): {best_thr:.3f}")
```
...
print(f"LPM prediction range: [{pred_lpm.min():.3f}, {pred_lpm.max():.3f}]")
print(f"Predictions below 0: {(pred_lpm < 0).sum()}")
print(f"Predictions above 1: {(pred_lpm > 1).sum()}")
