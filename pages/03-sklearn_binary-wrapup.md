...
| 이름 | 한 줄 설명 | 다음 챕터에서 |
|---|---|---|
| `sklearn.linear_model.LogisticRegression` | sigmoid + BCE 내장 이진/다중 분류 | Ch 4 softmax(2차원), Ch 5 multi-class로 확장 |
...
- **softmax**: 다중 클래스용 일반화. binary에 softmax를 쓰려면 출력 차원을 2로 늘리고 라벨도 one-hot으로 바꿔야 합니다 → 이게 정확히 Ch 4에서 다룰 "방식 B"입니다 (방식 A=sigmoid 1차원, 방식 B=softmax 2차원이 수학적으로 동등; BERT로는 Ch 11에서 재확인).
...
- Ch 3과 **똑같은 Yelp 이진화 데이터** 를 그대로 사용. 바뀌는 건 출력 표현과 loss 뿐.
- 출력이 1차원에서 **2차원** 으로 늘어나고, sigmoid 대신 **softmax**, `BCEWithLogitsLoss` 대신 **`CrossEntropyLoss`**.
- 핵심: 방식 A(sigmoid 1차원 + BCE)와 방식 B(softmax 2차원 + CE)가 **수학적으로 동등** 함을 `predict_proba` 일치로 직접 시연 ($z = z_1 - z_0$ 리파라미터화).
- 5클래스 multi-class 는 그다음 **Ch 5**, multi-label 은 **Ch 6** 으로 이어집니다.
