...
중앙값이 100단어, 평균 134단어인데 최대 977단어까지 긴 꼬리가 있습니다. 다만 BERT 가 세는 것은 단어가 아니라 WordPiece **토큰**이고, 이 5,000건은 평균 약 180토큰입니다. 그래서 이후 BERT 챕터에서 `max_length=128` 로 자르면 **절반가량이 뒷부분부터 잘립니다**. 잘리는 건 대개 리뷰 후반부라 별점 예측 영향은 크지 않지만, `max_length` 를 정할 때는 단어 수가 아니라 토큰 수 분포를 봐야 합니다 — Ch 8 에서 직접 확인합니다.
