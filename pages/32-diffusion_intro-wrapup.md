...
| 이름 | 한 줄 설명 | 다음 챕터에서 |
|---|---|---|
| `BertForMaskedLM(config)` (random init) | bidirectional encoder + MLM head, diffusion 의 denoiser | Ch 33 - 같은 본체 그대로, 샘플러만 교체 |
| `DiffusionCollator` (직접 구현) | 매 배치 `t ~ U(0,1)` 가변 마스킹 | Ch 33 동일 · Ch 34 한국어 |
| `1/t` 재가중 loss (`compute_loss` 오버라이드) | masked-diffusion denoising 목표 (log-likelihood bound) | (개념) LLaDA / MDLM 의 핵심 항 |
| `diffusion_generate` (low-confidence remasking) | 전부 `[MASK]` → 반복 denoise 생성 | Ch 33 - carry-over semi-AR + 반복억제로 개선 |
| `[MASK]` 토큰 (BPE 2048 에 special token 으로 추가, id 2) | forward (가리기) + reverse (생성) 의 캔버스 | Ch 33 동일 · Ch 34 한국어 vocab |
...
거칠기의 큰 몫은 *샘플러* 에도 있습니다 — Ch 33 에서 같은 모델에 *carry-over semi-AR + 반복억제* 샘플러를 적용해 반복을 없앱니다. 남는 규모 한계는 모델·데이터를 키워야 하지만(아래) T4 예산 밖입니다.
...
*반복을 없앤 깔끔한 생성* 은 Ch 33 에서 — 같은 모델에 *carry-over semi-AR + 반복억제* 샘플러를 적용하고, 고정-t accuracy·KL·4-gram 세 지표로 그 개선을 정량 확인합니다.
...
**Chapter 33. 작은 diffusion LM으로 영어 동화 끝까지 생성하기 — 샘플러 개선**

본 챕터에서 세운 *같은 작은 모델* (`BertForMaskedLM` 3.79M, BPE 2048, 30000 step, `1/t` loss)을 **그대로 두고**, 바꾸는 건 오직 **생성 샘플러** 하나입니다.

- **carry-over semi-AR + 반복억제** — 기본 confidence remasking 샘플러가 남기던 *반복* (`named named`, `play and play`)을, 확정 토큰을 다음 step으로 이어받는 carry-over 와 temperature·top-p·repetition penalty·인접 중복 금지로 없앱니다.
- **세 숫자로 정량 진단** — 눈대중 대신 ① 고정-t top-1 accuracy(모델이 조건부를 배웠나) ② KL(생성‖유니그램)(빈도를 넘어선 구조인가) ③ 4-gram 반복률(샘플러가 맴도나)로 개선을 측정합니다.

> **변하는 축**: *생성 샘플러* (모델·학습·loss 는 Ch 32 그대로). Ch 34 에서 *한국어 diffusion + autoregressive 직접 비교* 로 Phase 5 를 마무리합니다.
