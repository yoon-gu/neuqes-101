...
다만 본편 §5의 `sklearn`은 `max_features=20000` + bigram, 이 부록은 `TfidfVectorizer()` 기본값(제한 없음·unigram)이라 **설정이 다릅니다.** 본편의 점은 이 곡선 *위의 점*이 아니라 *같은 추세대에 놓인 별도의 측정*으로 읽어 주세요. 두 실험이 공유하는 결론은 특정 좌표가 아니라 **교차점의 위치와 그 위에서 격차가 완만하다는 추세**입니다.

## 보너스 — BERT 성능을 끌어올리는 5가지 lever

데이터는 가장 큰 lever 중 하나일 뿐입니다. 실무에서 분류 정확도를 더 올리고 싶을 때 순서대로 검토하는 카드입니다.
...
| **5. LLM 라벨링 + distill** | 큰 LLM으로 라벨 생성 → 작은 모델 증류 (2024-2025 트렌드) | +3-10%p |

이 부록은 그중 **1번(데이터 양)** 을 곡선으로 분리해 본 것입니다. 5클래스에서 본편의 근소한 격차가 *데이터 부족* 때문이라면 곡선이 우상향하며 벌어지고, *task 난이도* 때문이라면 큰 데이터에서도 격차가 평탄할 것입니다 — 위 결과 곡선이 그 답입니다.
