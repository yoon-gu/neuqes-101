...
print("\n=== conditional (prompt '옛날 옛날에' fixed) ===")
for i in range(3):
...
전부 `[MASK]`에서 시작한 무조건 생성인데도 인물(맥스·팀·수·안나)과 배경(공원·마당), 대화, 서사가 갖춰진 한국어 동화가 나옵니다. "맥스라는 큰 공"처럼 자잘한 흠은 4.29M 작은 모델의 한계이지만, 80/10/10 한 가지로 collapse를 벗어났다는 건 분명합니다.
