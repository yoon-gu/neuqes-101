...
위 진단에서 좁힌 범인(100% `[MASK]`)을 80/10/10으로 고친 콜레이터로 같은 모델·데이터를 학습합니다. T4에서 30000 step, 약 20분.
...
순진한 이식이 7.06(유니그램 벽)에서 멈췄던 것과 달리, 80/10/10 콜레이터로 바꾸자 train_loss가 baseline 8.29에서 4.13까지 내려갑니다. 콜레이터 한 줄을 고쳤을 뿐인데 모델이 비로소 문맥을 읽기 시작했다는 신호이고, 30000 step이 20분으로 T4 30분 예산 안에 들어왔습니다.
...
전부 `[MASK]`에서 시작한 무조건 생성인데도 인물(맥스·팀·수·안나)과 배경(공원·마당), 대화, 사건의 흐름이 갖춰진 한국어 동화가 나옵니다. 순진한 버전이 내놓던 `",. 함께 하루, 큰.!."` 파편과는 차원이 다릅니다. "맥스라는 큰 공", "큰 성"처럼 자잘한 흠은 4.29M 작은 모델의 한계이지만, 80/10/10 한 가지로 붕괴를 벗어났다는 건 분명합니다.
...
샘플러를 배제하고 모델만 잰 고정-t(0.15) top-1 정확도가 0.652입니다. 순진한 이식의 0.081(거의 찍기 수준)에서 8배 넘게 뛰었고, 영어 Ch 33의 0.717에 근접합니다. 가린 자리 100개 중 65개를 양방향 문맥만으로 복원한다는 뜻으로, 모델이 한국어의 조건부 구조를 실제로 학습했다는 직접 증거입니다.
