...
이번 장에서 작은 diffusion LM을 영어로 손수 살려냈다면, 다음 장에서는 똑같은 레시피를 **한국어**로 옮깁니다. Ch 34는 같은 4.29M짜리 mask-diffusion 모델로 한국어 동화(TinyStories-Korean)를 학습하는데, 영어에서 멀쩡하던 100% `[MASK]` 방식이 한국어에서는 조용히 무너집니다. 교착어라 조사·어미의 유니그램 예측성이 높아 "마스크 자리엔 흔한 토큰 찍기" 지름길의 유혹이 훨씬 강하기 때문입니다. 이 붕괴를 **BERT가 원래 쓰던 80/10/10 마스킹**(80% `[MASK]` / 10% 랜덤 토큰 / 10% 원본 유지) 한 가지로 되살리는 과정을 따라갑니다. 변화의 축은 "언어" 하나지만, 그 변화가 마스킹 방식의 숨은 결함을 끌어내는 장입니다.
