...
같은 본체로 sampling 설정만 바꿨는데 출력 결이 뚜렷이 달라집니다. `T=0.3`은 안전하지만 "새"와 "나는 너를 사랑해"를 되풀이하고, `T=1.0`/`T=1.2`로 갈수록 소재는 다양해지나 "젠프를 얻으라고", "새 총겼지만", "멋진 일을 하며요"처럼 어색한 표현이 섞입니다. 학습된 next-token 분포는 고정이고, 그 분포에서 *얼마나 넓게 뽑느냐*만 바뀐 결과입니다.
