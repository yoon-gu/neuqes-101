...
같은 본체·같은 prompt인데도 sampling 설정만 바꾸니 출력 성격이 달라집니다. `T=0.3`은 "새"를 반복하며 가장 안전하고 단조롭고, `T=1.0` nucleus와 `T=1.2`는 화제가 더 자유롭게 번지지만 "능으로 떨렸어요"처럼 어색한 토큰도 섞입니다. 사전학습된 분포는 그대로이고 표본 추출 방식만 바뀐 결과입니다.
