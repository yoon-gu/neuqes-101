# Hugging Face로 시작하는 언어모델: Colab 무료버전으로 만드는 BERT·GPT·Diffusion LM from scratch

* [01. 텍스트 벡터화 (TF-IDF)](pages/01-tfidf.md)
  * [01-1. 실습: Yelp 리뷰 데이터 살펴보기](pages/01-tfidf-practice.md)
  * [01-2. 해부: CountVectorizer — 텍스트를 "단어 횟수"로](pages/01-tfidf-anatomy.md)
  * [01-3. 변형: TfidfVectorizer — 흔한 단어의 영향력 줄이기](pages/01-tfidf-variation.md)
  * [01-4. 정리와 FAQ](pages/01-tfidf-wrapup.md)
* [02. 회귀 분석 (Regression & MSE)](pages/02-sklearn_regression.md)
  * [02-1. 실습: 별점 1-5를 그대로 회귀하기](pages/02-sklearn_regression-practice.md)
  * [02-2. 해부: "출력은 그냥 숫자다"](pages/02-sklearn_regression-anatomy.md)
  * [02-3. 변형: 별점을 [0, 1]로 정규화](pages/02-sklearn_regression-variation.md)
  * [02-4. 정리와 FAQ](pages/02-sklearn_regression-wrapup.md)
* [03. 이진 분류 (Binary Classification & BCE)](pages/03-sklearn_binary.md)
  * [03-1. 실습: `LogisticRegression`으로 이진 분류](pages/03-sklearn_binary-practice.md)
  * [03-2. 해부: sigmoid는 logit을 어떻게 확률로 바꾸나](pages/03-sklearn_binary-anatomy.md)
  * [03-3. 변형: 임계값(threshold)을 옮기면](pages/03-sklearn_binary-variation.md)
  * [03-4. 정리와 FAQ](pages/03-sklearn_binary-wrapup.md)
* [04. sigmoid와 softmax의 동등성 (Binary Classification)](pages/04-softmax_binary.md)
  * [04-1. 실습: 두 방식을 나란히 학습](pages/04-softmax_binary-practice.md)
  * [04-2. 해부: 수학적 동등성](pages/04-softmax_binary-anatomy.md)
  * [04-3. 변형: sklearn은 왜 K=2 multinomial에서 `(2, V)` coef를 안 만드나?](pages/04-softmax_binary-variation.md)
  * [04-4. 정리와 FAQ](pages/04-softmax_binary-wrapup.md)
* [05. 다중 클래스 분류 (Multi-class Classification & CE)](pages/05-sklearn_multiclass.md)
  * [05-1. 실습: 5클래스 분류](pages/05-sklearn_multiclass-practice.md)
  * [05-2. 해부: 평가 지표 한꺼번에 보기](pages/05-sklearn_multiclass-anatomy.md)
  * [05-3. 변형: multinomial vs OvR](pages/05-sklearn_multiclass-variation.md)
  * [05-4. 정리와 FAQ](pages/05-sklearn_multiclass-wrapup.md)
* [06. 다중 라벨 분류 (Multi-label Classification & Per-label BCE)](pages/06-sklearn_multilabel.md)
  * [06-1. 실습: 항목 라벨 합성](pages/06-sklearn_multilabel-practice.md)
  * [06-2. 해부: multi-label 평가 지표](pages/06-sklearn_multilabel-anatomy.md)
  * [06-3. 변형: 라벨별 임계값(threshold) 변경](pages/06-sklearn_multilabel-variation.md)
  * [06-4. 정리와 FAQ](pages/06-sklearn_multilabel-wrapup.md)
* [07. BERT 파이프라인 (Pipeline)](pages/07-bert_pipeline.md)
  * [07-1. 실습: 환경 준비](pages/07-bert_pipeline-practice.md)
  * [07-2. 해부: pipeline 안에서는 뭐가 일어났을까?](pages/07-bert_pipeline-anatomy.md)
  * [07-3. 변형: pipeline 없이 직접 해보기](pages/07-bert_pipeline-variation.md)
  * [07-4. 정리와 FAQ](pages/07-bert_pipeline-wrapup.md)
* [08. 토크나이저와 데이터셋 (Tokenizer & Datasets)](pages/08-tokenizer_datasets.md)
  * [08-1. 실습: `datasets` 로 Yelp 로드](pages/08-tokenizer_datasets-practice.md)
  * [08-2. 해부: 토크나이저 옵션 직접 실험](pages/08-tokenizer_datasets-anatomy.md)
  * [08-3. 변형: `datasets.map` — 5,000건 일괄 토큰화](pages/08-tokenizer_datasets-variation.md)
  * [08-4. 정리와 FAQ](pages/08-tokenizer_datasets-wrapup.md)
* [09. BERT 회귀 분석 (Regression & Trainer)](pages/09-bert_regression.md)
  * [09-1. 실습: 데이터 준비](pages/09-bert_regression-practice.md)
  * [09-2. 해부: 평가 — sklearn(Ch 2)과 직접 비교](pages/09-bert_regression-anatomy.md)
  * [09-3. 변형: 학습이 어디서 망가지는지 (개념만)](pages/09-bert_regression-variation.md)
  * [09-4. 정리와 FAQ](pages/09-bert_regression-wrapup.md)
* [10. BERT 이진 분류: Sigmoid (BCE)](pages/10-bert_binary_sigmoid.md)
  * [10-1. 실습: 데이터 — Yelp 이진화 (Ch 3·4와 동일)](pages/10-bert_binary_sigmoid-practice.md)
  * [10-2. 해부: 평가 — sigmoid 확률 분포 직접 확인](pages/10-bert_binary_sigmoid-anatomy.md)
  * [10-3. 정리와 FAQ](pages/10-bert_binary_sigmoid-wrapup.md)
* [11. BERT 이진 분류: Softmax (CE)](pages/11-bert_binary_softmax.md)
  * [11-1. 실습: 데이터 — Yelp 이진화 (Ch 10과 정확히 동일)](pages/11-bert_binary_softmax-practice.md)
  * [11-2. 해부: 평가 — softmax 확률 분포](pages/11-bert_binary_softmax-anatomy.md)
  * [11-3. 변형: 클라이맥스 — 방식 A 를 *이 노트북 안에서* 다시 학습해 비교](pages/11-bert_binary_softmax-variation.md)
  * [11-4. 정리와 FAQ](pages/11-bert_binary_softmax-wrapup.md)
* [12. BERT 다중 클래스 분류 (Multi-class & CE)](pages/12-bert_multiclass.md)
  * [12-1. 실습: 데이터 — Yelp 별점 1-5 (Ch 5와 동일)](pages/12-bert_multiclass-practice.md)
  * [12-2. 해부: 평가 — softmax 확률 분포와 혼동 패턴](pages/12-bert_multiclass-anatomy.md)
  * [12-3. 변형: 클라이맥스 — sklearn TF-IDF + LogReg 와의 비교 (Ch 5의 BERT 검증)](pages/12-bert_multiclass-variation.md)
  * [12-4. 정리와 FAQ](pages/12-bert_multiclass-wrapup.md)
  * [12-5. 부록 — 데이터 스케일링 곡선](pages/12-bert_multiclass-data_scaling.md)
* [13. BERT 다중 라벨 분류 (Multi-label & Per-label BCE)](pages/13-bert_multilabel.md)
  * [13-1. 실습: 데이터 — Yelp + 항목(aspect) 합성 라벨 (Ch 6과 동일)](pages/13-bert_multilabel-practice.md)
  * [13-2. 해부: 평가 — 라벨별 sigmoid 확률 + 활성 패턴](pages/13-bert_multilabel-anatomy.md)
  * [13-3. 변형: 클라이맥스 — sklearn `OneVsRestClassifier(LogisticRegression)` 와 비교](pages/13-bert_multilabel-variation.md)
  * [13-4. 정리와 FAQ](pages/13-bert_multilabel-wrapup.md)
* [14. 보조 손실과 멀티태스크 학습 (Auxiliary Loss)](pages/14-auxiliary_loss.md)
  * [14-1. 실습: 데이터 — Yelp + 항목 (Ch 13) + 별점 보조 라벨](pages/14-auxiliary_loss-practice.md)
  * [14-2. 해부: 평가 — 메인 task + 보조 task](pages/14-auxiliary_loss-anatomy.md)
  * [14-3. 변형: 클라이맥스 — *λ=0 baseline* 학습 (= Ch 13 재현)](pages/14-auxiliary_loss-variation.md)
  * [14-4. 정리와 FAQ](pages/14-auxiliary_loss-wrapup.md)
  * [14-5. 부록 — λ 스윕으로 sweet spot 찾기](pages/14-auxiliary_loss-lambda_sweep.md)
* [15. 한국어 BERT 이진 분류 (Korean Binary Classification)](pages/15-ko_binary.md)
  * [15-1. 실습: 토크나이저 비교 — 같은 한국어 문장, 두 토크나이저](pages/15-ko_binary-practice.md)
  * [15-2. 해부: 평가 — softmax 확률 분포](pages/15-ko_binary-anatomy.md)
  * [15-3. 정리와 FAQ](pages/15-ko_binary-wrapup.md)
* [16. 한국어 BERT 다중 클래스 분류 (Korean Multi-class Classification)](pages/16-ko_multiclass.md)
  * [16-1. 실습: 데이터 — KLUE-YNAT (뉴스 헤드라인 7분류)](pages/16-ko_multiclass-practice.md)
  * [16-2. 해부: 평가 — softmax 확률 분포 + 혼동 패턴](pages/16-ko_multiclass-anatomy.md)
  * [16-3. 정리와 FAQ](pages/16-ko_multiclass-wrapup.md)
* [17. 한국어 BERT 다중 라벨 분류 (Korean Multi-label Classification)](pages/17-ko_multilabel.md)
  * [17-1. 실습: 데이터 — KLUE-YNAT 결합으로 multi-label 합성](pages/17-ko_multilabel-practice.md)
  * [17-2. 해부: 평가 — 카테고리별 sigmoid 확률 + 공동 활성 패턴](pages/17-ko_multilabel-anatomy.md)
  * [17-3. 변형: 합성 샘플 직접 읽기 + threshold 옮겨보기](pages/17-ko_multilabel-variation.md)
  * [17-4. 정리와 FAQ](pages/17-ko_multilabel-wrapup.md)
* [18. 한국어 BERT 보조 손실 (Korean Auxiliary Loss)](pages/18-ko_auxiliary.md)
  * [18-1. 실습: 데이터 — KLUE-YNAT 합성 multi-label + 활성 개수 보조 라벨](pages/18-ko_auxiliary-practice.md)
  * [18-2. 해부: 평가 — 메인 task + 보조 task](pages/18-ko_auxiliary-anatomy.md)
  * [18-3. 변형: 클라이맥스 — *λ=0 baseline* 학습 (= Ch 17 재현)](pages/18-ko_auxiliary-variation.md)
  * [18-4. 정리와 FAQ](pages/18-ko_auxiliary-wrapup.md)
  * [18-5. 부록 — λ 스윕으로 약한 보조 신호의 sweet spot 찾기](pages/18-ko_auxiliary-lambda_sweep.md)
* [19. 토크나이저 직접 학습 (Tokenizer Training)](pages/19-tokenizer_training.md)
  * [19-1. 실습: 환경 셋업](pages/19-tokenizer_training-practice.md)
  * [19-2. 해부: 같은 문장을 4 토크나이저로 비교](pages/19-tokenizer_training-anatomy.md)
  * [19-3. 변형: vocab 크기 sweep](pages/19-tokenizer_training-variation.md)
  * [19-4. 정리와 FAQ](pages/19-tokenizer_training-wrapup.md)
* [20. 작은 BERT 사전학습 (English MLM Pretraining)](pages/20-en_bert_pretrain.md)
  * [20-1. 실습: 환경 셋업](pages/20-en_bert_pretrain-practice.md)
  * [20-2. 해부: 평가 — MLM loss 곡선 + perplexity + masked token 예측](pages/20-en_bert_pretrain-anatomy.md)
  * [20-3. 변형: 학습 step 더 늘리거나 block_size 변경](pages/20-en_bert_pretrain-variation.md)
  * [20-4. 정리와 FAQ](pages/20-en_bert_pretrain-wrapup.md)
  * [20-5. 부록 — 사전학습량과 perplexity 곡선](pages/20-en_bert_pretrain-scaling.md)
* [21. 작은 BERT 이진 분류 (English Yelp Fine-tuning)](pages/21-en_bert_classify.md)
  * [21-1. 실습: 환경 셋업](pages/21-en_bert_classify-practice.md)
  * [21-2. 해부: 평가 — Ch 10 과 같은 5종 metric + 학습 곡선](pages/21-en_bert_classify-anatomy.md)
  * [21-3. 정리와 FAQ](pages/21-en_bert_classify-wrapup.md)
* [22. 작은 BERT 사전학습 (Korean MLM Pretraining)](pages/22-ko_bert_pretrain.md)
  * [22-1. 실습: 환경 셋업](pages/22-ko_bert_pretrain-practice.md)
  * [22-2. 해부: 학습 결과 — Loss / Perplexity 곡선](pages/22-ko_bert_pretrain-anatomy.md)
  * [22-3. 변형: 데이터 / 학습량 / 다른 한국어 코퍼스](pages/22-ko_bert_pretrain-variation.md)
  * [22-4. 정리와 FAQ](pages/22-ko_bert_pretrain-wrapup.md)
* [23. 작은 BERT 이진 분류 (Korean NSMC Fine-tuning)](pages/23-ko_bert_classify.md)
  * [23-1. 실습: 환경 셋업](pages/23-ko_bert_classify-practice.md)
  * [23-2. 해부: 평가 — Ch 15 / Ch 21 과 같은 5종 metric + 학습 곡선](pages/23-ko_bert_classify-anatomy.md)
  * [23-3. 정리와 FAQ](pages/23-ko_bert_classify-wrapup.md)
* [24. 작은 GPT 사전학습 (TinyStories Causal LM)](pages/24-gpt_tinystories.md)
  * [24-1. 실습: 환경 셋업](pages/24-gpt_tinystories-practice.md)
  * [24-2. 변형: sampling hyperparam 비교](pages/24-gpt_tinystories-variation.md)
  * [24-3. 정리와 FAQ](pages/24-gpt_tinystories-wrapup.md)
* [25. GPT-2 계속 사전학습 (Continual Pretraining)](pages/25-gpt2_continual_pretrain.md)
  * [25-1. 실습: 환경 셋업](pages/25-gpt2_continual_pretrain-practice.md)
  * [25-2. 변형: 더 많은 epoch / 다른 도메인 / catastrophic forgetting 시연](pages/25-gpt2_continual_pretrain-variation.md)
  * [25-3. 정리와 FAQ](pages/25-gpt2_continual_pretrain-wrapup.md)
* [26. 한국어 작은 GPT 사전학습 (TinyStories-Korean Causal LM)](pages/26-ko_tiny_gpt.md)
  * [26-1. 실습: 환경 셋업](pages/26-ko_tiny_gpt-practice.md)
  * [26-2. 변형: sampling hyperparam / 더 큰 모델 / 더 많은 stories](pages/26-ko_tiny_gpt-variation.md)
  * [26-3. 정리와 FAQ](pages/26-ko_tiny_gpt-wrapup.md)
* [27. KoGPT2 계속 사전학습 (Korean Continual Pretraining)](pages/27-ko_gpt2_continual_pretrain.md)
  * [27-1. 실습: 환경 셋업](pages/27-ko_gpt2_continual_pretrain-practice.md)
  * [27-2. 변형: 더 많은 epoch / 다른 도메인 / catastrophic forgetting 시연](pages/27-ko_gpt2_continual_pretrain-variation.md)
  * [27-3. 정리와 FAQ](pages/27-ko_gpt2_continual_pretrain-wrapup.md)
* [28. KoGPT2 SFT (Instruction Tuning)](pages/28-sft.md)
  * [28-1. 실습: 환경 셋업](pages/28-sft-practice.md)
  * [28-2. 변형: collator 의 `labels` 마스킹 직접 시각화 — **이 챕터의 클라이맥스**](pages/28-sft-variation.md)
  * [28-3. 정리와 FAQ](pages/28-sft-wrapup.md)
* [29. 분야별 벤치마크 평가 (Benchmark Evaluation)](pages/29-benchmark_eval.md)
  * [29-1. 실습: 환경 셋업](pages/29-benchmark_eval-practice.md)
  * [29-2. 해부: 해석 — 작은 모델의 한계, scaling, 벤치마크 오염](pages/29-benchmark_eval-anatomy.md)
  * [29-3. 정리와 FAQ](pages/29-benchmark_eval-wrapup.md)
* [30. DPO: 사람 선호로 정렬 (Preference Alignment)](pages/30-dpo.md)
  * [30-1. 실습: 환경 셋업](pages/30-dpo-practice.md)
  * [30-2. 변형: β 조정 / 더 많은 preference / DPO 변종](pages/30-dpo-variation.md)
  * [30-3. 정리와 FAQ](pages/30-dpo-wrapup.md)
* [31. GRPO: 검증 가능한 보상으로 정렬 (Verifiable Reward)](pages/31-grpo.md)
  * [31-1. 실습: 환경 셋업](pages/31-grpo-practice.md)
  * [31-2. 변형: group size / format reward / 코드 verifier / 다른 task](pages/31-grpo-variation.md)
  * [31-3. 정리와 FAQ](pages/31-grpo-wrapup.md)
* [32. 작은 Diffusion LM 입문: 병렬 Denoise 생성](pages/32-diffusion_intro.md)
  * [32-1. 실습: 환경 셋업](pages/32-diffusion_intro-practice.md)
  * [32-2. 변형: 조건부 생성 (prompt 고정)](pages/32-diffusion_intro-variation.md)
  * [32-3. 정리와 FAQ](pages/32-diffusion_intro-wrapup.md)
* [33. Diffusion LM 샘플러 교정: 반복 억제 생성](pages/33-diffusion_train.md)
  * [33-1. 실습: 환경 셋업](pages/33-diffusion_train-practice.md)
  * [33-2. 해부: 모델이 정말 조건부를 배웠나 (눈대중 말고 숫자로)](pages/33-diffusion_train-anatomy.md)
  * [33-3. 변형: 샘플러만 바꿔보기](pages/33-diffusion_train-variation.md)
  * [33-4. 정리와 FAQ](pages/33-diffusion_train-wrapup.md)
* [34. 한국어 Diffusion LM: 80/10/10 마스킹](pages/34-ko_diffusion.md)
  * [34-1. 실습: 직접 학습하고 생성해 보기](pages/34-ko_diffusion-practice.md)
  * [34-2. 변형: 결과 — coherent 생성과 정확도 0.652](pages/34-ko_diffusion-variation.md)
  * [34-3. 정리와 FAQ](pages/34-ko_diffusion-wrapup.md)
