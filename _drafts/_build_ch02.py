"""Build 02_sklearn_regression/02_sklearn_regression.ipynb."""
import json
from pathlib import Path

REPO = Path(__file__).resolve().parent.parent
OUT = REPO / "02_sklearn_regression" / "02_sklearn_regression.ipynb"
...
# ----- 1. Title -----
md(r"""# Chapter 2. sklearn Regression — 첫 모델, 첫 Loss
...
# ----- 13. MSE 직접 계산 -----
code(r"""# sklearn의 mean_squared_error가 내부에서 뭘 계산하는지 직접 재현
...
# ----- 15. normalized regression -----
code(r"""y_train_norm = (y_train - 1) / 4   # 1-5 → 0-1
...
`LinearRegression`은 최소제곱법으로 한 번에 풀기 때문에 라벨 스케일이 결과에 영향을 거의 주지 않습니다. 위 셀에서 보았듯 정규화 공간에서 학습한 모델을 원래 별점 공간으로 되돌리면 정규화 없이 학습한 모델과 거의 같은 MSE가 나옵니다.
...
힌트: 최소제곱법 해에서 라벨에 상수를 곱하면 가중치도 같은 비율로 바뀔 뿐, 예측을 같은 스케일로 되돌리면 정확히 같은 값이 나와야 합니다.""")
