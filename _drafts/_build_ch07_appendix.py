...
print(f"Parent class:  {type(cfg).__mro__[1].__name__}")
...
print(f"d_model:             {t5_cfg.d_model}            (hidden state dim)")
print(f"d_ff:                {t5_cfg.d_ff}             (FFN inner dim)")
print(f"num_layers:          {t5_cfg.num_layers}             (encoder layers)")
print(f"num_decoder_layers:  {t5_cfg.num_decoder_layers}  (decoder layers)")
print(f"num_heads:           {t5_cfg.num_heads}")
print(f"is_encoder_decoder:  {t5_cfg.is_encoder_decoder}    (← False for BERT/GPT-2)")""")
...
print(f"Class:             {type(vit_cfg).__name__}")
print(f"model_type:        {vit_cfg.model_type}")
print(f"image_size:        {vit_cfg.image_size}    (input image side, pixels)")
print(f"patch_size:        {vit_cfg.patch_size}     (image → patch side)")
...
print()
print(f"hasattr(vocab_size)?  {hasattr(vit_cfg, 'vocab_size')}    (← only text models have vocab)")""")
...
print(f"Updated num_labels:   {model.config.num_labels}")
print(f"Updated id2label:     {model.config.id2label}")
print(f"Updated problem_type: {model.config.problem_type}")
print()
print(f"Classification head shape: {model.classifier.weight.shape}  (5 x 768 — newly randomized)")""")
...
print(f"Dict key count: {len(cfg_dict)}")
...
print(f"Reloaded num_labels:    {reloaded_config.num_labels}")
print(f"Reloaded id2label:      {reloaded_config.id2label}")
