...
| 축 | Ch 5 | Ch 6 |
|---|---|---|
| 모델 구성 | softmax 한 번 | **K개 독립 sigmoid** (`OneVsRestClassifier`) |
| Activation | softmax (합 = 1) | **per-label sigmoid** (라벨끼리 독립) |
| Loss | `CrossEntropyLoss` | **per-label `BCEWithLogitsLoss` 평균** |
| 라벨 형식 | int 한 개 (0-4) | **multi-hot float 벡터** (예: `[1, 0, 1, 0, 1]`) |
| 데이터 | 별점 5클래스 | **Yelp + 측면 키워드 합성** (5개 측면) |
| 토크나이저 | TF-IDF | TF-IDF (그대로) |

핵심 변화는 **상호배타 가정의 해제**. multi-class에서는 별점 1과 별점 5가 동시에 활성될 수 없지만, multi-label에서는 한 리뷰가 음식·서비스·가격을 동시에 다룰 수 있습니다. softmax의 합=1 제약이 그대로 남아 있으면 이 상황을 표현할 수 없으므로, K개 sigmoid가 라벨마다 *독립적으로* 0/1을 결정합니다.

Ch 5에서 본 OvR의 사고방식이 그대로 재활용됩니다 — 다만 이번엔 마지막에 정규화하지 않습니다 (그게 multi-label의 본래 모습).""")
...
# ----- 12. 학습 도입 -----
md(r"""## 🚀 실습 2: `OneVsRestClassifier` 학습

Ch 5에서 본 `OneVsRestClassifier`를 그대로 multi-label에 적용합니다. 차이는 **`Y_train`의 shape이 (N,)에서 (N, K)** 로 바뀌었다는 것뿐 — sklearn이 multi-label 모드로 자동 인식하고 K개 binary 분류기를 독립적으로 학습합니다.""")
