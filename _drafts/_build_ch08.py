...
# ----- 18. max_length 해석 -----
md(r"""**해석**: 학습 시 `max_length=128` 로 두면 **약 절반이 잘립니다** — 위 출력의 평균(177)이 중앙값(131)보다 큰 것은 소수의 매우 긴 리뷰가 평균을 끌어올리는 긴 꼬리 분포이기 때문입니다. 잘리는 건 대부분 리뷰 뒷부분의 세부 묘사라 별점 예측 성능에는 영향이 작지만, "대부분 온전히 담긴다" 는 표현은 정확하지 않습니다. `max_length=512` 면 대부분을 보존하지만 평균 패딩 비율이 60-70%라 메모리·시간 낭비.
...
Yelp는 평균 177 토큰이라 `max_length=128` 이면 **약 절반이 잘립니다**(이 챕터 실측 50.9%). 그래도 잘리는 건 대부분 리뷰 뒷부분이라 별점 예측에는 영향이 작아 표준값으로 씁니다. 더 긴 문서가 많은 데이터(예: 논문 abstract)는 `256` 또는 `512` 가 안전.
