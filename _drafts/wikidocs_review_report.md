...
> **⚠️ 처리 현황 (2026-06-24 갱신)** — 이 리포트는 2026-06-21 스냅샷입니다(원문 보존). **위 60건은 산문 영역에서 거의 전부 반영 완료**(커밋 `a56abbf` → `4f5522b` → `c94f758`)되었고, 2차 검수에서 **리포트에 없던 신규 이슈**도 추가로 처리했습니다. 처리 현황·신규 발견·미해결(⏱ 시간 추정치, 💻 코드 영역)은 **문서 맨 끝 "처리 현황 및 2차 검수 추가" 섹션** 참조.
...
## 처리 현황 및 2차 검수 추가 (2026-06-24)

> 위 본문은 2026-06-21 스냅샷(원문 보존). 아래는 그 60건의 처리 결과 + 2차 검수(실행본 전수 재대조)에서 **리포트에 없던 신규 발견** + 미해결 항목입니다.

### A. 위 60건 처리 현황

- **반영 완료(산문)** — 커밋 `a56abbf`(60건 1차 일괄) → `4f5522b`(잔여 산문) → `c94f758`(2차 검수) 에 걸쳐 처리:
  - 높음: Ch1 Phase번호 / Ch2·3 다음 챕터 예고 / Ch6·7·8 cross-ref(산문) / Ch11·12 파라미터 합계(검증 결과 이미 정확) / Ch18 `CLAUDE.md` 누출 / Ch21·22·23 기대치·데이터·epoch / Ch32 토크나이저·모델·FAQ Q6 — **완료**.
  - 중간: '전체 20챕터'→'전체 챕터'(Ch1-18) / Phase 3 범위(Ch16-18) / alignment Ch30-31(Ch26·27) / Ch16·17 토큰 길이 / Ch19 물결표 / Ch20·21 Yelp 도메인 / Ch20·21 loss·ppl / Ch23 고아 89% / β=0.1(Ch30) / Ch31 헤딩·링크 / Ch33·34 예고 메타 — **완료**.
  - 낮음: Ch15 자모 / Ch19 Wikitext / Ch20 블록수 / Ch24 Phase4·loss / Ch28 3K / Ch31 §2.5·4.5 / Ch33 4-gram 0.177 / Ch34 0.081·20.20분 — **완료**.
- **미해결(의도적 보류 / pages 밖)**:
  - ⏱ **학습 시간 표기**(#7·#33·#34·#56·#57·#59·#75·#76 등): 실행본 elapsed 가 비정상적으로 빠름(학습 0.2~1분 — colab-cli 환경 추정). "예상 N분"이 학습자용 T4 추정인지 실측 반영인지 **결정 필요 → 보류**. 단 **Ch23 은 같은 파일에 8-10분/0.2분 공존(자가당착)** 이라 어느 쪽이든 통일 필요.
  - 💻 **코드 영역(노트북 수정 필요)**: Ch6 `# PyTorch (Ch12…)` / Ch8 `# Ch9 회귀…multi-label` 주석·`# Ch13 보조 loss` / Ch23 FAQ Q1 스니펫(Yelp 변수) / Ch30 β 주석 / Ch31 `ref-free` print / Ch32 `bs=32` plot title / Ch34 `'Once upon a time'` 헤더. pages/ 로는 수정 불가.
  - 🔤 **print() 한국어 출력**(#11, 21파일 59곳): 코드/출력 영역 → 노트북 수정 사안. **미처리**.
  - 🧮 **DistilBERT 66M/67M**(#9): 책 전체 공통이며 "헤드 포함 67M"이 대체로 맞아 **보류**.

### B. 2차 검수 신규 발견 — 리포트에 없던 것 (모두 처리: `c94f758` + Ch3 후속)

| Ch | 위치 | 문제 | 처리 |
|---|---|---|---|
| 1 | variation | TF-IDF idf 식에 sklearn `smooth_idf` `+1` 누락(같은 챕터 Q3와 모순) | `+1` 추가 |
| 1 | wrapup | `datasets` "Ch 7에서 깊게"→Ch 8 / `TfidfVectorizer` "Ch 2-5"→Ch 2-6 | 정정 |
| 2 | wrapup | "Ch 8 BERT 회귀"(2곳) — BERT 회귀=Ch 9 | →Ch 9 |
| 3 | wrapup | **다음 챕터 예고 통째로 Ch 5(multi-class) 내용** — Ch 4는 softmax binary | Ch 4 예고로 재작성 |
| 4 | wrapup | "Ch 11 BERT multi-class" — multi-class=Ch 12 | →Ch 12 |
| 5 | anatomy | MSE 페널티 "1 vs 16" — 4★→1★ 거리 3 → 3²=9 | →1 vs 9 |
| 7 | overview | problem_type "Ch 9·11·12" — single_label=Ch12, multi_label=Ch13 | →Ch 9·12·13 |
| 8 | overview+wrapup | `max_length=128` "95% 정상, 5% 잘림" — 실측 **50.9% 잘림**(평균 177) | 약 51%로 정정 |
| 14 | overview | "보조가 도움/quick 모드/food·service 양의 delta" — 실측 **전 항목 음수**(micro 0.82→0.64, macro 0.76→0.34, full 모드 λ=1) | 정직화·quick 모드 표현 제거 |
| 18 | wrapup | "GPT 챕터(Ch 21)" — Ch 21은 영어 BERT 분류 | →Ch 24 이후 GPT |
| 19 | overview+wrapup | 한국어 WordLevel UNK "5-15%" — 실측 **43.74%** | ~43.7%로 정정 |
| 21 | overview | MLM 코퍼스 "5K(70만~100만 토큰)" — 실제 **2K paragraphs(~27만)** | 2K로 정정 |
| 22 | wrapup | 데이터 구성 `from_pandas(df["document"])` "Ch 15 패턴" — Ch 22는 `Dataset.from_dict`(분류 챕터 잔재) | 실제 패턴으로 |
| 25 | wrapup | "Ch 28 SFT 에서 KoGPT2 등장" — KoGPT2=Ch 27 | →Ch 27 |
| 26 | overview+wrapup | 파라미터 "약 3M"(다수) — 실측 **4.22M**; "125M=40배"→30배 | 4.2M·30배 |
| 27 | overview+wrapup | "EOS 를 pad 로 재활용/`if None` 가드" — 실제 별도 `pad_token="<pad>"`; loss/PPL 도달점 표 stale(실측 2.49/~3000 step) | 정정 |
| 28 | overview | SFT 전 출력 "동화체" — 실제 **블로그·해시태그·SNS 잡담체** | 정정 |
| 29 | practice | "오답일수록 log-prob 낮아져" — sum 만 단조, mean 은 순서 뒤집힘 | sum 한정 |
| 32 | overview+wrapup | **Ch 33 을 "MDLM 170M / DiffuGPT 124M 사전학습"으로 오기**(각 7곳) — 실제 Ch 33은 샘플러 챕터; "30,000 stories"→실제 100,000 | 정정 |

### C. 리포트 수치 자체 정정(실행본 재실행으로 값이 갱신됨)

- **Ch 21**(#29·#54): 제안치 'acc 0.65 / AUC 0.71'이 아니라 실측 **acc 0.6260 / AUC 0.6821**(리포트의 0.6490은 사실 **F1**). Ch 10 인용은 약 0.90 유지.
- **Ch 23**(#31·row 17/181/183): 실측 **acc 0.5480 / AUC 0.5545**. MLM epoch 자가당착은 **이미 3으로 통일** 완료.
- **Ch 22**: 한국어 MLM perplexity 실측 **~1,853**(리포트 미기재).
- **Ch 3 방식 B**(중간 #45): 리포트는 'Ch 11'을 제안했으나, 직전 챕터 맥락상 sklearn 방식 B 첫 등장은 **Ch 4** — "Ch 4에서 다룰 방식 B (BERT로는 Ch 11에서 재확인)"으로 정정.
- **Ch 14**(신규): 리포트엔 없던 개요의 'quick 모드/일부 항목 향상' 낙관 서술이 실측(전 항목 음수)과 정면 모순이라 정직화.