...
## Loss
**`CrossEntropyLoss`** — Ch 4 sklearn `LogisticRegression(multi_class="multinomial")` 과 같은 식. 다른 점은 logit 출처가 BERT 768-dim hidden state.
...
## 환경
Google Colab **T4 GPU 필수**. 약 10분.

**선행 조건**: Ch 10 노트북을 *같은 Colab 세션* 에서 미리 끝까지 돌려 `./shared_binary_results/` 가 만들어져 있어야 5장 비교가 동작합니다. 세션이 끊겼다면 Drive 백업 또는 Ch 10 재실행 필요.
