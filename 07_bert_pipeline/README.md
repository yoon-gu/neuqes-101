...
[![Open In Colab — 본 노트북](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/yoon-gu/neuqes-101/blob/master/07_bert_pipeline/07_bert_pipeline.ipynb)
[![Open In Colab — 부록: model.config 깊이 보기](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/yoon-gu/neuqes-101/blob/master/07_bert_pipeline/appendix_model_config.ipynb)
...
📒 **[appendix_model_config.ipynb](./appendix_model_config.ipynb)** — `model.config` 깊이 보기. `PretrainedConfig` 클래스 계층, `AutoConfig.from_pretrained` 로 가중치 없이 config만 로드, 5종 모델(BERT/DistilBERT/GPT-2/T5/RoBERTa) + ViT(비전) config 비교, 분류 헤드 갈아끼우는 `from_pretrained` 인자 패턴. 본 챕터 흐름과는 별도, 시간 될 때 추천. [공식 문서](https://huggingface.co/docs/transformers/en/main_classes/configuration).
