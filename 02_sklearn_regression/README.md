...
## 다루는 핵심 개념
- `LinearRegression` 최소제곱법 풀이 (p > n 이라 정규방정식의 역행렬은 없음)
- 첫 손실 함수로 **평균제곱오차(`MSELoss`)** 등장 — 수식과 직관, sklearn `mean_squared_error`로 직접 재현
...
| **2** | `LinearRegression()` | `TfidfVectorizer()` | Yelp (별점 1-5) | `MSELoss` |
