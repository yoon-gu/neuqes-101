...
| 27 | [![Open](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/yoon-gu/neuqes-101/blob/master/27_ko_gpt2_continual_pretrain/27_ko_gpt2_continual_pretrain.ipynb) | KoGPT2 (사전학습) — **continual pretraining** (Ch 25의 한국어판, *같은 CausalLM task 를 한국어 데이터로 더 학습*. SFT 아님) | Character BPE (KoGPT2 그대로, SentencePiece 스타일 — Ch 26 byte-level BBPE와 다른 계열) | `g0ster/TinyStories-Korean` (Ch 26과 동일) | `Linear(H,V)` (LM head 그대로) | softmax | `CrossEntropyLoss` (next-token) | shifted token ids |
| 28 | [![Open](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/yoon-gu/neuqes-101/blob/master/28_sft/28_sft.ipynb) | KoGPT2 SFT | Character BPE (KoGPT2 그대로) | KoAlpaca (instruction → response) | `Linear(H,V)` | softmax | `CrossEntropyLoss` (`SFTTrainer`, response-only mask 옵션) | chat template token ids |
...
| 30 | [![Open](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/yoon-gu/neuqes-101/blob/master/30_dpo/30_dpo.ipynb) | Qwen2.5-0.5B-Instruct (KoGPT2 에서 교체) + frozen ref | BBPE (Qwen2.5) | preference 쌍 (chosen/rejected) | `Linear(H,V)` | log-likelihood ratio | `DPO sigmoid loss` (`DPOTrainer`) | (chosen, rejected) pair |
| 31 | [![Open](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/yoon-gu/neuqes-101/blob/master/31_grpo/31_grpo.ipynb) | Qwen2.5-0.5B-Instruct (SFT 없이, KoGPT2 에서 교체) | BBPE (Qwen2.5) | verifiable-reward prompts (글자 세기 char count) | `Linear(H,V)` + group advantage | softmax | `GRPO loss` (group relative, `GRPOTrainer`) | rollout group + verifier |
